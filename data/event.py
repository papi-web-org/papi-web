...
class Event:
    ...
    def __build_family(self, family_id: str):
        ...
        try:
            ...
            self.reader.add_warning('option absente, famille ignorée', section_key, key)
            return
        ...
        if family_indices is None:
            ...
            return
        self.reader.add_debug(f"valeurs : {' '.join(family_indices)}", section_key, key)
        for screen_index in family_indices:
            ...
            for sub_section_key, properties in template.data.items():
                ...
                self.reader.setdefault(new_section_key, {})
                self.reader.add_debug(f"ajout de la rubrique [{new_section_key}]", section_key)
    ...
    def __build_screen_sets(self, section_keys: List[str], columns: int) -> List[ScreenSet]:
        ...
        for section_key in section_keys:
            try:
                ...
                self.reader.add_error('rubrique non trouvée', section_key)
                return screen_sets
            key = 'tournament'
            ...
            if key in current_section:
                first = self.reader.getint_safe(section_key, key, minimum=1)
                if first is None:
                    self.reader.add_warning(
            ...
            if key in current_section:
                last = self.reader.getint_safe(section_key, key)
                if last is None:
                    self.reader.add_warning(
                        'un entier positif non nul est attendu, ignoré',
                        section_key,
            ...
            if key in current_section:
                part = self.reader.getint_safe(section_key, key)
                if part is None:
                    self.reader.add_warning(
                        'un entier positif non nul est attendu, ignoré',
                        section_key,
            ...
            if key in current_section:
                parts = self.reader.getint_safe(section_key, key)
                if parts is None:
                    self.reader.add_warning(
                        'un entier positif non nul est attendu, ignoré',
                        section_key,
    ...
    def __build_rotator(self, rotator_id: str):
        ...
        if key not in rotator_section:
            ...
            delay = self.reader.getint_safe(section_key, key, minimum=1)
            if delay is None:
                self.reader.add_warning(
    ...
    def __build_timer_hour(self, hour_id: str, timer: Timer):
        ...
        if key not in timer_section:
            self.reader.add_warning('option absente, horaire ignoré', section_key, key)
            return
        ...
        if timestamp is None:
            self.reader.add_warning(f'date [{datetime_str}] non valide ([YYYY-MM-DD hh:mm] ou [hh:mm] attendu), '
        ...
        if timer.hours:
            ...
            if timestamp <= previous_hour.timestamp:
                self.reader.add_warning(f"l'horaire [{hour.datetime_str}] arrive avant l'horaire précédent "
    ...
    def __build_timer_delays(self, timer: Timer):
        ...
        for key in delay_section:
            if key not in section_keys:
                ...
                continue
            delay_id = int(key)
            delay: Optional[int] = self.reader.getint_safe(section_key, key, minimum=1)
            if delay is None:
                self.reader.add_warning(
