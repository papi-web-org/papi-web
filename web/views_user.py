...
class EventUserWebContext(UserWebContext):
    def __init__(
        ...
        self.user_event = EventLoader.get(request=self.request, lazy_load=lazy_load).load_event(user_event_uniq_id)
        if self.user_event.errors:
            self._redirect_error(self.user_event.errors)
    ...
    def _user_index_update_needed(
        ...
        for public_event in event_loader.public_events:
            web_context: EventUserWebContext = EventUserWebContext(
                request, {'user_event_uniq_id': public_event.uniq_id, }, True)
