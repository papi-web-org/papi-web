...
class Event(ConfigReader):
    ...
    def __build_root(self):
        ...
        try:
            ...
            self.reader._add_debug('option absente', section_key, key)
    ...
    def __build_tournaments(self):
        ...
        if self.reader.has_section('tournament'):
            ...
            self.reader._add_error('aucun tournoi trouvé', 'tournament.*')
            return
        for tournament_id in tournament_ids:
            self.__build_tournament(tournament_id)
        if not len(self.__tournaments):
            self.reader._add_error('aucun tournoi initialisé')
    ...
    def __build_tournament(self, tournament_id: str):
        ...
        try:
            ...
            self.reader._add_warning('un entier est attendu', section_key, key)
            ffe_id = None
    ...
    def _get_value_with_warning(
        ...
        *messages,
        ...
        try:
            ...
            self.reader._add_error(messages[0], section_key)
            return default_value
        except KeyError:
            self.reader._add_warning(messages[1], section_key, key)
            return default_value
        except ValueError:
            self.reader._add_warning(messages[2], section_key, key)
            return default_value
        except AssertionError:
    ...
    def __build_templates(self):
        ...
        if not len(self.__templates):
            self.reader._add_debug('aucun modèle initialisé')
    ...
    def __build_screens(self):
        ...
        if not len(self.__screens):
            self.reader._add_warning("aucun écran n'a été initialisé")
